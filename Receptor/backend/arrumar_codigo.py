...
from .conversores import bits_para_decimal, texto_para_bits, bits_para_texto ,decimal_para_bits ,padronizar_bits
...
from .bytecode import get_bytecode , get_bits_array
...
def arrumar_codigo(sinal , tipo_enquadramento, deteccao_erro):
    ...
    print(f"Processando sinal modulado")
    ...
    if deteccao_erro == "Código de Hamming":
        ...
        texto = str(desenquadramento)

        # GerarGrafico(texto)
        get_bytecode(texto)

        texto = desenquadramento
        bits = get_bits_array(texto)
        ...
        print(f"verificar: {desenquadramento}")
